...
data_dir = 'D:\\TALCS_corpus\\data_aishell'
annotation_path = 'D:\\TALCS_corpus\\data_aishell'

parser = argparse.ArgumentParser(description=__doc__)
add_arg = functools.partial(add_arguments, argparser=parser)
add_arg("filepath", default='D:\\', type=str, help="压缩包data_aishell.tgz文件路径,不指定会自动下载")
add_arg("target_dir", default=None, type=str, help="存放音频文件的目录")
add_arg("annotation_text", default=None, type=str, help="存放音频标注文件的目录")
...
def create_annotation_text(data_dir, annotation_path):
    ...
    transcript_path = os.path.join(data_dir, 'transcript', 'label.txt')
    transcript_dict = {}
